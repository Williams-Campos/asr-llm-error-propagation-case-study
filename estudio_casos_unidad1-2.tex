...
\begin{longtable}{|p{\textwidth}|@{}c@{}}
  ...
  cuantización, $|\varepsilon_i|\le d/2$. Cada capa del modelo evalúa productos punto de longitud $n$
  (típicamente $n\ge 512$). Bajo el modelo estándar de aritmética de punto flotante,
  $\mathrm{fl}(a\circ b)=(a\circ b)(1+\delta)$ con $|\delta|\le u$, el error acumulado de un producto punto
  satisface la cota clásica&\\
  \vspace{-4pt}
  \[
  \left|\mathrm{fl}\!\left(\sum_{i=1}^{n} a_i b_i\right)-\sum_{i=1}^{n} a_i b_i\right|
  \;\le\; \gamma_n \sum_{i=1}^{n}|a_i||b_i|,
  \qquad \gamma_n=\frac{nu}{1-nu},
  \]
  \vspace{-14pt}&\\
  a lo que se suma el error de cuantización de los propios coeficientes. Estos errores no permanecen
  aislados: se propagan a través de decenas de capas, atraviesan operaciones sensibles como
  \texttt{softmax} y \texttt{logsumexp} (donde puede aparecer \textit{cancelación catastrófica} al restar
  cantidades cercanas), y luego se realimentan en la \textbf{decodificación autorregresiva}, en la que cada
  token generado condiciona a los siguientes. Finalmente, la salida de la etapa 1 es la entrada de la
  etapa 2, por lo que la cascada compone dos operadores y, en primera aproximación, sus números de
  condición se multiplican, $\kappa_{\text{total}} \approx \kappa_{\text{LLM}}\cdot\kappa_{\text{ASR}}$.&\\
  Un detalle adicional relevante: la suma en punto flotante \textbf{no es asociativa}, por lo que cambiar el
  número de hilos de ejecución (\texttt{-t} en \texttt{whisper.cpp}) altera el orden de las reducciones y puede
  producir salidas distintas para la misma entrada y el mismo modelo.&\\
  ...
  resultados en un informe y una presentación.&\\&\\\hline
\end{longtable}
